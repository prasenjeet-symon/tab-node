...
export class ArticleStoryDistributionCollection {
  ...
  public async fetchAllActiveStories(userDocID: string): Promise<{ [key: string]: MArticleStoryDistribution.IArticleStoryDistribution[] }> {
    try {
      const snap = await this.database.listDocuments(this.databaseID, AppwriteCollection.ARTICLE_STORIES_DISTRIBUTION, [
      ...
      const groupedStories = finalStories.reduce((acc, cur) => {
        const userDocId = cur.story ? cur.story.doc.user.docID : 'no_story';
  ...
  public async fetchAllStoryViewers(story: MArticleStory.IArticleStory): Promise<MArticleStoryDistribution.IArticleStoryDistribution[]> {
    try {
      const snap = await this.database.listDocuments(this.databaseID, AppwriteCollection.ARTICLE_STORIES_DISTRIBUTION, [
        Query.equal('story_docID', story.id),
        Query.equal('isSeen', true),
        Query.orderDesc('boostPoint'),
      ]);
