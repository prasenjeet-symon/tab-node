...
export class CommentsCollection {
    ...
    public async addNewComment(comment: MArticleComment.IArticleComment) {
        try {
            await this.database.createDocument(this.databaseID, AppwriteCollection.COMMENTS, comment.id, serializeAppwriteData(comment.doc), [
                Permission.write(Role.user(comment.doc.commentedBy.docID)),
                Permission.delete(Role.user(comment.doc.commentedBy.docID)),
                Permission.update(Role.user(comment.doc.commentedBy.docID)),
    ...
    public async fetchComments(articleDocID: string, prevCursor: MArticleComment.IArticleComment | undefined, limit: number) {
        try {
            const snap = prevCursor
                ? await this.database.listDocuments(this.databaseID, AppwriteCollection.COMMENTS, [Query.equal('article_docID', articleDocID), Query.equal('parentComment_docID', ''), Query.orderDesc('createdAt'), Query.cursorAfter(prevCursor.id), Query.limit(+limit)])
                : await this.database.listDocuments(this.databaseID, AppwriteCollection.COMMENTS, [Query.equal('article_docID', articleDocID), Query.equal('parentComment_docID', ''), Query.orderDesc('createdAt'), Query.limit(+limit)]);
    ...
    public async fetchChildComments(parentCommentDocID: string, prevCursor: MArticleComment.IArticleComment | undefined, limit: number) {
        try {
            const snap = prevCursor
                ? await this.database.listDocuments(this.databaseID, AppwriteCollection.COMMENTS, [Query.equal('parentComment_docID', parentCommentDocID), Query.orderDesc('createdAt'), Query.cursorAfter(prevCursor.id), Query.limit(+limit)])
                : await this.database.listDocuments(this.databaseID, AppwriteCollection.COMMENTS, [Query.equal('parentComment_docID', parentCommentDocID), Query.orderDesc('createdAt'), Query.limit(+limit)]);
    ...
    /** Fetch number of comments of given article */
    public async fetchCommentsCount(articleDocID: string) {
        try {
            const snap = await this.database.listDocuments(this.databaseID, AppwriteCollection.COMMENTS, [Query.equal('article_docID', articleDocID)]);
