...
export class UserCollection {
    ...
    public async createUser(user: MUser.IUser) {
        ...
        try {
            await this.database.createDocument(this.databaseID, AppwriteCollection.USERS, user.id, serializeAppwriteData(user.doc), [Permission.delete(Role.user(user.id)), Permission.update(Role.user(user.id)), Permission.read(Role.any())]);
        } catch (error) {
