import { AppwriteCollection, AppwriteErrorReporterNodeJs, AppwriteNodeJsClient, ArticleBoostPoints, MArticle, MArticleDistribution, MArticleTopicRelationship, MSavedArticle, MTopic, MUser, MUserRelationship, UserBoostPoints, deserializeAppwriteData, serializeAppwriteData } from '@tabnode/utils';
import { produce } from 'immer';
import { Query } from 'node-appwrite';

/** Increase the boost point for the event creator */
export async function increaseBoostPointOfUser(aSaved: MSavedArticle.ISavedArticle) {
...
export async function incDecBoostPointOfArticle(aSaved: MSavedArticle.ISavedArticle) {
  try {
    const client = new AppwriteNodeJsClient();
    const database = client.database();
    const databaseID = client.databaseID();
...
export async function increaseDecreaseBoostPointOfArticleDistribution(aSaved: MSavedArticle.ISavedArticle) {
  const client = new AppwriteNodeJsClient();
  const database = client.database();
  const databaseID = client.databaseID();
...
export async function increaseDecreaseRelationshipStrengthOfAuthorAndUser(aSaved: MSavedArticle.ISavedArticle) {
...
export async function IncreaseDecreaseRelatedTopicsBoostPoint(aSaved: MSavedArticle.ISavedArticle) {
  // database connection
  const client = new AppwriteNodeJsClient();
  const database = client.database();
  const databaseID = client.databaseID();
