...
import { AWFunction, AppwriteCollection, ArticleBoostPoints, MArticle, MArticleDistribution, MArticleLike, MArticleRelationSuggestion, MArticleTopicRelationship, MTopic, MUser, MUserRelationSuggestion, MUserRelationship, UserBoostPoints, deserializeAppwriteData, serializeAppwriteData } from '@tabnode/utils';
import { produce } from 'immer';
import { Permission, Query, Role } from 'node-appwrite';

/** Increase the boost point for the event creator */
export async function increaseBoostPointOfUser(req: AWFunction.Req, aSuggestion: MArticleRelationSuggestion.IArticleRelationSuggestion) {
...
export async function incDecBoostPointOfArticle(req: AWFunction.Req, aSuggestion: MArticleRelationSuggestion.IArticleRelationSuggestion) {
    try {
        const client = new AppwriteNodeJsClient(req);
        const database = client.database();
        const databaseID = client.databaseID();
...
export async function increaseDecreaseBoostPointOfArticleDistribution(req: AWFunction.Req, aSuggestion: MArticleRelationSuggestion.IArticleRelationSuggestion) {
    const client = new AppwriteNodeJsClient(req);
    const database = client.database();
    const databaseID = client.databaseID();
...
export async function increaseDecreaseRelationshipStrengthOfAuthorAndUser(req: AWFunction.Req, aSuggestion: MArticleRelationSuggestion.IArticleRelationSuggestion) {
...
export async function IncreaseDecreaseRelatedTopicsBoostPoint(req: AWFunction.Req, aSuggestion: MArticleRelationSuggestion.IArticleRelationSuggestion) {
    // database connection
    const client = new AppwriteNodeJsClient(req);
    const database = client.database();
    const databaseID = client.databaseID();
...
export async function suggestUserAuthorToFollow(req: AWFunction.Req, aSuggestion: MArticleRelationSuggestion.IArticleRelationSuggestion) {
    // database connection
    const client = new AppwriteNodeJsClient(req);
    const database = client.database();
    const databaseID = client.databaseID();
    ...
    const followSuggestion = await fetchFollowSuggestion(aSuggestion.doc.for, fullArticle.doc.writer);
